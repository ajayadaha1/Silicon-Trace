...
with st.sidebar:
    ...
    st.markdown("Upload Excel files with hardware asset data.")
    ...
    if uploaded_files:
        ...
        if st.button("🚀 Process Files", type="primary", use_container_width=True):
            ...
            # Display results
            if success_count > 0:
                st.success(f"✓ Processed {success_count}/{len(uploaded_files)} file(s)")
                st.metric("Total Rows", total_processed)
            ...
            if failed_files:
                st.error(f"✗ Failed: {len(failed_files)} file(s)")
